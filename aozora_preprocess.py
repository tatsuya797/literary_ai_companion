...
def text_cleanse_df(df):
    ...
    # 本文の開始位置をauthor_nameの1行上に変更
    if head_tx == []:
        # もし'---…'区切りが無い場合、作家名の直前の行を探す
        head_tx = list(df[df['text'].str.contains(author_name)].index)
        head_tx_num = head_tx[0] - 1  # 作家名の1行上
    else:
        # 2個目の'---…'区切り直後から本文が始まる
        head_tx_num = head_tx[1] + 1

    # 本文の開始行から終了行までのデータを取得
    df_e = df[head_tx_num + 1:atx[0]]

    # タイトル行を取得（author_nameの1行上の行）
    title = df.loc[head_tx_num, 'text']
